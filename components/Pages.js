import { useEscapeToClose } from 'hooks'

export default ({ close }) => {
  useEscapeToClose(close)

  return (
    <div
      className="div-block-861-copy"
      style={{
        position: 'absolute',
        width: '100%',
        top: 0,
        background: 'white',
        zIndex: 1,
      }}
    >
      <div className="div-block-876-copy _40">
        <div className="form-block w-form">
          <form id="email-form" name="email-form" data-name="Email Form">
            <input
              type="text"
              className="text-field w-input"
              autoFocus
              maxLength={256}
              name="name"
              data-name="Name"
              placeholder="Search"
              id="name"
            />
          </form>
          <div className="w-form-done">
            <div>Thank you! Your submission has been received!</div>
          </div>
          <div className="w-form-fail">
            <div>Oops! Something went wrong while submitting the form.</div>
          </div>
        </div>
      </div>
      <div className="div-block-876-copy-copy">
        <div className="text-block-118 edia">
          Command Shift N to create new column.{' '}
        </div>
      </div>
      <div className="div-blorgck-436-copy ren">
        <div className="div-block-828">
          <div className="tab raf show">
            <div className="div-block-784 kcalm">
              <div className="_141">
                <div className="bluh">
                  What Are the Symptoms...
                  <br />
                </div>
              </div>
            </div>
            <div className="div-block-784 kcalm">
              <img
                src="https://uploads-ssl.webflow.com/5eaf5cd658f15e7f0410a7cd/5eaf5cd658f15eb99010a952_arrow-right.svg"
                width={19}
                alt=""
                className="image-269"
              />
              <div className="_141">
                <div className="bluh">
                  Diarrhea
                  <br />
                </div>
              </div>
            </div>
            <div className="div-block-784 kcalm">
              <img
                src="https://uploads-ssl.webflow.com/5eaf5cd658f15e7f0410a7cd/5eaf5cd658f15eb99010a952_arrow-right.svg"
                width={19}
                alt=""
                className="image-269"
              />
              <div className="_141">
                <div className="bluh">
                  Diarrhea
                  <br />
                </div>
              </div>
            </div>
            <div className="div-block-784 kcalm">
              <img
                src="https://uploads-ssl.webflow.com/5eaf5cd658f15e7f0410a7cd/5eaf5cd658f15eb99010a952_arrow-right.svg"
                width={19}
                alt=""
                className="image-269"
              />
              <div className="_141">
                <div className="bluh">
                  Diarrhea
                  <br />
                </div>
              </div>
            </div>
            <div className="div-block-784 kcalm">
              <img
                src="https://uploads-ssl.webflow.com/5eaf5cd658f15e7f0410a7cd/5eaf5cd658f15eb99010a952_arrow-right.svg"
                width={19}
                alt=""
                className="image-269"
              />
              <div className="_141">
                <div className="bluh">
                  Diarrhea
                  <br />
                </div>
              </div>
            </div>
            <div className="div-block-784 kcalm">
              <img
                src="https://uploads-ssl.webflow.com/5eaf5cd658f15e7f0410a7cd/5eaf5cd658f15eb99010a952_arrow-right.svg"
                width={19}
                alt=""
                className="image-269"
              />
              <div className="_141">
                <div className="bluh">
                  Diarrhea
                  <br />
                </div>
              </div>
            </div>
            <div className="div-block-827">
              <div className="text-block-199">Untitled Folder</div>
            </div>
          </div>
        </div>
        <div className="div-block-828">
          <div className="tab raf show">
            <div className="div-block-784 kcalm">
              <div className="_141">
                <div className="bluh">
                  Sleep deprivation
                  <br />
                </div>
              </div>
            </div>
            <div className="div-block-784 kcalm">
              <img
                src="https://uploads-ssl.webflow.com/5eaf5cd658f15e7f0410a7cd/5eaf5cd658f15eb99010a952_arrow-right.svg"
                width={19}
                alt=""
                className="image-269"
              />
              <div className="_141">
                <div className="bluh">
                  Diarrhea
                  <br />
                </div>
              </div>
            </div>
            <div className="div-block-784 kcalm">
              <img
                src="https://uploads-ssl.webflow.com/5eaf5cd658f15e7f0410a7cd/5eaf5cd658f15eb99010a952_arrow-right.svg"
                width={19}
                alt=""
                className="image-269"
              />
              <div className="_141">
                <div className="bluh">
                  Diarrhea
                  <br />
                </div>
              </div>
            </div>
            <div className="div-block-784 kcalm">
              <img
                src="https://uploads-ssl.webflow.com/5eaf5cd658f15e7f0410a7cd/5eaf5cd658f15eb99010a952_arrow-right.svg"
                width={19}
                alt=""
                className="image-269"
              />
              <div className="_141">
                <div className="bluh">
                  Diarrhea
                  <br />
                </div>
              </div>
            </div>
            <div className="div-block-827">
              <div className="text-block-199">D</div>
            </div>
          </div>
          <div className="tab raf show">
            <div className="div-block-784 kcalm">
              <div className="_141">
                <div className="bluh">
                  Sleep deprivation
                  <br />
                </div>
              </div>
            </div>
            <div className="div-block-784 kcalm">
              <img
                src="https://uploads-ssl.webflow.com/5eaf5cd658f15e7f0410a7cd/5eaf5cd658f15eb99010a952_arrow-right.svg"
                width={19}
                alt=""
                className="image-269"
              />
              <div className="_141">
                <div className="bluh">
                  Diarrhea
                  <br />
                </div>
              </div>
            </div>
            <div className="div-block-784 kcalm">
              <img
                src="https://uploads-ssl.webflow.com/5eaf5cd658f15e7f0410a7cd/5eaf5cd658f15eb99010a952_arrow-right.svg"
                width={19}
                alt=""
                className="image-269"
              />
              <div className="_141">
                <div className="bluh">
                  Diarrhea
                  <br />
                </div>
              </div>
            </div>
            <div className="div-block-784 kcalm">
              <img
                src="https://uploads-ssl.webflow.com/5eaf5cd658f15e7f0410a7cd/5eaf5cd658f15eb99010a952_arrow-right.svg"
                width={19}
                alt=""
                className="image-269"
              />
              <div className="_141">
                <div className="bluh">
                  Diarrhea
                  <br />
                </div>
              </div>
            </div>
            <div className="div-block-827">
              <div className="text-block-199">E</div>
            </div>
          </div>
          <div className="tab raf show">
            <div className="div-block-784 kcalm">
              <div className="_141">
                <div className="bluh">
                  Sleep deprivation
                  <br />
                </div>
              </div>
            </div>
            <div className="div-block-784 kcalm">
              <img
                src="https://uploads-ssl.webflow.com/5eaf5cd658f15e7f0410a7cd/5eaf5cd658f15eb99010a952_arrow-right.svg"
                width={19}
                alt=""
                className="image-269"
              />
              <div className="_141">
                <div className="bluh">
                  Diarrhea
                  <br />
                </div>
              </div>
            </div>
            <div className="div-block-784 kcalm">
              <img
                src="https://uploads-ssl.webflow.com/5eaf5cd658f15e7f0410a7cd/5eaf5cd658f15eb99010a952_arrow-right.svg"
                width={19}
                alt=""
                className="image-269"
              />
              <div className="_141">
                <div className="bluh">
                  Diarrhea
                  <br />
                </div>
              </div>
            </div>
            <div className="div-block-784 kcalm">
              <img
                src="https://uploads-ssl.webflow.com/5eaf5cd658f15e7f0410a7cd/5eaf5cd658f15eb99010a952_arrow-right.svg"
                width={19}
                alt=""
                className="image-269"
              />
              <div className="_141">
                <div className="bluh">
                  Diarrhea
                  <br />
                </div>
              </div>
            </div>
            <div className="div-block-827">
              <div className="text-block-199">F</div>
            </div>
          </div>
        </div>
      </div>
      <div className="div-block-780 ewgew-copy">
        <a href="#" className="lohd w-inline-block">
          <img
            src="https://uploads-ssl.webflow.com/5eaf5cd658f15e7f0410a7cd/5eaf5cd658f15e071b10a974_icons8-sail-60.png"
            width={32}
            alt=""
          />
        </a>
        <div className="avtars">
          <a className="link-block w-inline-block" onClick={close}>
            <div className="text-block-207 _6hu">twrite</div>
            <div className="text-block-207 yfg">
              <img
                src="https://uploads-ssl.webflow.com/5eaf5cd658f15e7f0410a7cd/5eb42e3c726ea04428c2ca2d_x%20(1).svg"
                width={32}
                alt=""
                className="image-284"
              />
              <div className="text-block-208">Sleep deprivation</div>
            </div>
          </a>
          <div className="div-block-872" />
        </div>
        <div className="div-block-892">
          <div className="kerou lefa">
            <img
              src="https://uploads-ssl.webflow.com/5eaf5cd658f15e7f0410a7cd/5eb72240fb1f1e55d1b73b72_folder-plus-2.svg"
              width={26}
              alt=""
              className="image-288"
            />
            <div className="text-block-212 eg">New Folder </div>
          </div>
        </div>
      </div>
      <div className="tab raf show sf">
        <div className="tob titl">
          <div className="_141">
            <div>
              A-Z
              <br />
            </div>
          </div>
        </div>
        <div className="tob">
          <div className="_141">
            <div>
              Date Modified
              <br />
            </div>
          </div>
        </div>
        <div className="tob">
          <div className="_141">
            <div>
              Date Created
              <br />
            </div>
          </div>
        </div>
        <div className="tob">
          <div className="_141">
            <div>
              Categories
              <br />
            </div>
          </div>
        </div>
        <div className="tob">
          <div className="_141">
            <div>
              Word
              <br />
            </div>
          </div>
        </div>
      </div>
    </div>
  )
}
